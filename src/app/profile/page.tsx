...
export default function ProfilePage() {
  ...
  if (loading) {
    return <div className="px-4 py-6 max-w-md mx-auto"><div className="h-32 bg-[#111111] rounded-2xl animate-pulse border border-[#1a1a1a]" /></div>
  }
  ...
  return (
    <div className="px-4 py-6 max-w-md mx-auto">
      <div className="flex items-center gap-4 mb-8">
        {user.photoURL ? (
          ...
          <div className="w-16 h-16 rounded-full bg-[#C9A84C]/20 flex items-center justify-center">
            <UserIcon size={28} className="text-[#C9A84C]" />
          </div>
        )}
        <div>
      ...
      <div className="flex flex-col gap-2">
        <Link href="/bookings"
          className="flex items-center gap-3 bg-[#111111] border border-[#1a1a1a] rounded-xl px-4 py-3.5 text-white hover:border-[#C9A84C]/30 transition-colors">
          <CalendarDays size={18} className="text-[#C9A84C]" />
          <span className="text-sm">My Bookings</span>
        </Link>
        <button onClick={signOut}
          className="flex items-center gap-3 bg-[#111111] border border-[#1a1a1a] rounded-xl px-4 py-3.5 text-red-400 hover:border-red-400/30 transition-colors">
          <LogOut size={18} />
